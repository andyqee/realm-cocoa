...
- (void)testObjectAggregate
{
    ...
    XCTAssertEqual(min.intValue, 1);
    min = [yesArray minOfProperty:@"intCol"];
    XCTAssertEqual(min.intValue, 0);

    // Test float min
    min = [noArray minOfProperty:@"floatCol"];
    XCTAssertEqualWithAccuracy(min.floatValue, 0.0f, 0.1f);
    min = [yesArray minOfProperty:@"floatCol"];
    XCTAssertEqualWithAccuracy(min.floatValue, 1.2f, 0.1f);

    // Test double min
    min = [noArray minOfProperty:@"doubleCol"];
    XCTAssertEqualWithAccuracy(min.doubleValue, 2.5, 0.1f);
    min = [yesArray minOfProperty:@"doubleCol"];
    XCTAssertEqualWithAccuracy(min.doubleValue, 0.0, 0.1f);

    // Test date min
    NSDate *dateMinOutput = [noArray minOfProperty:@"dateCol"];
    XCTAssertEqualWithAccuracy(dateMinOutput.timeIntervalSince1970, dateMaxInput.timeIntervalSince1970, 1);
    dateMinOutput = [yesArray minOfProperty:@"dateCol"];
    XCTAssertEqualWithAccuracy(dateMinOutput.timeIntervalSince1970, dateMinInput.timeIntervalSince1970, 1);

    // Test invalid column name
    XCTAssertThrows([noArray minOfProperty:@"foo"]);

    // Test operation not supported
    XCTAssertThrows([noArray minOfProperty:@"boolCol"]);


    // MAX ::::::::::::::::::::::::::::::::::::::::::::::
    // Test int max
    NSNumber *max = [noArray maxOfProperty:@"intCol"];
    XCTAssertEqual(max.integerValue, 1);
    max = [yesArray maxOfProperty:@"intCol"];
    XCTAssertEqual(max.integerValue, 0);

    // Test float max
    max = [noArray maxOfProperty:@"floatCol"];
    XCTAssertEqualWithAccuracy(max.floatValue, 0.0f, 0.1f);
    max = [yesArray maxOfProperty:@"floatCol"];
    XCTAssertEqualWithAccuracy(max.floatValue, 1.2f, 0.1f);

    // Test double max
    max = [noArray maxOfProperty:@"doubleCol"];
    XCTAssertEqualWithAccuracy(max.doubleValue, 2.5, 0.1f);
    max = [yesArray maxOfProperty:@"doubleCol"];
    XCTAssertEqualWithAccuracy(max.doubleValue, 0.0, 0.1f);

    // Test date max
    NSDate *dateMaxOutput = [noArray maxOfProperty:@"dateCol"];
    XCTAssertEqualWithAccuracy(dateMaxOutput.timeIntervalSince1970, dateMaxInput.timeIntervalSince1970, 1);
    dateMaxOutput = [yesArray maxOfProperty:@"dateCol"];
    XCTAssertEqualWithAccuracy(dateMaxOutput.timeIntervalSince1970, dateMinInput.timeIntervalSince1970, 1);

    // Test invalid column name
    XCTAssertThrows([noArray maxOfProperty:@"foo"]);

    // Test operation not supported
    XCTAssertThrows([noArray maxOfProperty:@"boolCol"]);
}
